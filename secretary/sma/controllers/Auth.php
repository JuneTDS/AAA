...
class Auth extends MY_Controller
{
    function __construct()
    {
        ...
        $this->load->library(array('form_validation', 'encryption', 'ion_auth', 'session'));
    ...
    function check_session(){
       ...
       $fiveMinutesBefore = date("YmdHi", "-10 seconds");
       echo json_encode($lastVisitTime);*/
       //echo ((date("YmdHi", strtotime($lastVisitTime)) > $fiveMinutesBefore)? 1 : 0);

        //$this->auth_model->trigger_events('logged_in');
        //echo ( (bool)$this->session->userdata('identity'));
       //echo ($this->loggedIn);
...
//        $user_id = $CI->session->userdata('user_id');
       ...
       //echo json_encode($CI->config->config["sess_expiration"]);
       // if($user_id)
       // {
       //       echo true;
       // }
    ...
    function client()
    {
        ...
        {
            $this->db->select($this->db->dbprefix('users').".id as id, first_name, last_name,".$this->db->dbprefix('users').".email," . $this->db->dbprefix('groups') . ".description, active, officer.name")
            ->from("users")
            ->join('groups', 'users.group_id = groups.id', 'inner')
            ->join('user_person', 'user_person.user_id = users.id AND user_person.user_admin_code_id = "'.$this->session->userdata("user_admin_code_id").'"', 'inner')
            ->join('officer', 'officer.id = user_person.officer_id', 'left')
            //->join('firm', 'firm.user_id = "'.$this->session->userdata("user_id").'"', 'left')
    ...
    function get_manager_name()
    {
        ...
        $admin_query = 'SELECT users.id, users.last_name, users.first_name, users.group_id FROM users where users.first_name = "TZE KARN" AND users.last_name = "KONG" AND users.id != 1 AND  users.user_deleted = 0 AND users.active = 1';
    ...
    public function get_rules_data()
    {
        ...
        echo json_encode($this->data['rules_info']);
    }
    ...
    function profile($id = NULL, $part = NULL)
    {
        ...
        $meta = array('page_title' => lang('profile'),  'page_name' => lang('profile'), 'bc' => $bc);
    ...
    function singpass_login($auth_code)
    {
        if ($this->loggedIn) {
            //redirect('welcome');
        ...
        $id_no_result = $this->ion_auth->singpass_login($auth_code);
        if ($id_no_result["boolean"])
        {
            $id_no = $id_no_result["id"];
            ...
            $q = $this->db->query('select users.id, users.first_name, users.last_name, users.email, users.group_id, users.user_admin_code_id, user_admin_code.admin_code from users LEFT JOIN user_admin_code on user_admin_code.id = users.user_admin_code_id where identification_no = "'.$id_no.'" and user_deleted = 0');
            $q = $q->result_array();

            $this->session->set_userdata('user_id', $q[0]["id"]);
            $this->session->set_userdata('first_name', $q[0]["first_name"]);
            $this->session->set_userdata('last_name', $q[0]["last_name"]);
            $this->session->set_userdata('email_for_user', $q[0]["email"]);
            $this->session->set_userdata('user_admin_code_id', $q[0]["user_admin_code_id"]);
            
            $this->save_audit_trail($this->session->userdata("user_id"), "", "Login", $q[0]["first_name"]." ".$q[0]["last_name"]." use SingPass Logged in.");
            ...
            //echo json_encode($id_no_result["id"]);
            $this->session->set_flashdata('error_title', "singpass_issue");
            redirect('login');
        }
    }
    ...
    function login($m = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
            {
                ...
                $q = $this->db->query('select users.id, users.first_name, users.last_name, users.email, users.group_id, users.user_admin_code_id, user_admin_code.admin_code from users LEFT JOIN user_admin_code on user_admin_code.id = users.user_admin_code_id where email = "'.$email.'" and user_deleted = 0');
                $q = $q->result_array();
                ...
                $this->save_audit_trail($this->session->userdata("user_id"), "", "Login", $q[0]["first_name"]." ".$q[0]["last_name"]." Logged in.");
    ...
    {
        $this->save_audit_trail($this->session->userdata("user_id"), "", "Logout", $this->session->userdata('first_name')." ".$this->session->userdata('last_name')." Logged out.");
    ...
    function profile_change_password($part = NULL)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($change) {
                ...
                $users_result = $this->db->query("select * from users where id = '".$_POST["user_id"]."'");
                $users_array = $users_result->result_array();

                if($id != $this->session->userdata('user_id') && $part != "access_client")
                {
                    $this->save_audit_trail($this->session->userdata("user_id"), "User", "Edit User", $users_array[0]['first_name']." ".$users_array[0]['last_name']." password is changed.");
                ...
                {
                    $this->save_audit_trail($this->session->userdata("user_id"), "User Profile", "Password", $users_array[0]['first_name']." ".$users_array[0]['last_name']." password is changed.");
    ...
    function forgot_password()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $user_q = $this->db->query('select users.id, users.first_name, users.last_name, users.email, users.group_id, users.user_admin_code_id, user_admin_code.admin_code from users LEFT JOIN user_admin_code on user_admin_code.id = users.user_admin_code_id where email = "'.$identity->email.'" and user_deleted = 0');
            $user_arr = $user_q->result_array();

            $forgotten = $this->ion_auth->forgotten_password($identity->email);

            if ($forgotten) {
                $this->session->set_flashdata('message', $this->ion_auth->messages());
                $this->save_audit_trail($user_arr[0]["id"], "", "Forgot Password", $user_arr[0]['first_name']." ".$user_arr[0]['last_name']." Reset password.");
                redirect("login#after_forgot_password");
            } else {
                $this->session->set_flashdata('error', $this->ion_auth->errors());
                redirect("login#forgot_password");
            }
    ...
    function get_person()
    {
        $this->db->select('officer.*, officer_email.email');
        $this->db->from('officer');
        $this->db->join('officer_email', 'officer_email.officer_id = officer.id and officer_email.primary_email = 1', 'left');
        $this->db->where('officer.user_admin_code_id', $this->session->userdata('user_admin_code_id'));
        $this->db->order_by('officer.name', 'asc');
        $q = $this->db->get();
        $result = $q->result_array();
        ...
        for($j = 0; $j < count($result); $j++)
        {
            //$res[$result[$j]['id']] = $this->encryption->decrypt($result[$j]['name']);
            $result[$j]['name'] = $this->encryption->decrypt($result[$j]['name']);
            $result[$j]['email'] = $this->encryption->decrypt($result[$j]['email']);
            $result[$j]['identification_no'] = $this->encryption->decrypt($result[$j]['identification_no']);
        }    

        $data = array('status'=>'success', 'tp'=>1, 'msg'=>"Person fetched successfully.", 'result'=>$result);
    ...
    function get_firm()
    {
        ...
        for($j = 0; $j < count($result); $j++)
        {
            ...
            {
                $res[$result[$j]['id']] = $result[$j]['name'].' ('.$result[$j]['branch_name'].')';
            }
            else
            {
    ...
    function create_access_client_user()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $selected_person = $this->input->post("person");
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $user_type_id, $password, $email, $additional_data, $term_and_condition, $active, $notify, $selected_client, $selected_person)) {
...
/*                'biller_id' => $this->input->post('biller'),
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $user_type_id, $password, $email, $additional_data, $term_and_condition, $active, $notify, $selected_firm)) {
            ...
            $this->save_audit_trail($this->session->userdata("user_id"), "User", "Create User", strtoupper($this->input->post('first_name'))." ".strtoupper($this->input->post('last_name'))." user is added.");
            ...
            $meta = array('page_title' => lang('users'), 'bc' => $bc, 'page_name' => "User Profile");
    ...
    public function reset_password_by_admin()
    {
        ...
        if ($userID == null) {
            ...
            $users = $this->db->query("SELECT * FROM users WHERE `id` = ".$userID.";");
            $users->num_rows();
            $users = $users->result_array();
        }

        // print_r($users);
        $newlyResetedPassword = $this->randomPassword();
        ...
        foreach ($users as $key => $value)
        {
            ...
            $change = $this->ion_auth->reset_password($identity, $newlyResetedPassword);
...
$config = SendinBlue\Client\Configuration::getDefaultConfiguration()->setApiKey('api-key', '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');
                //Xin Yee API
                ...
                $sendSmtpEmail['sender'] = json_decode('{"email":"looi@acumenbizcorp.com.sg"}', true);
                $sendSmtpEmail['to'] = json_decode('[{"email":"'.$identity.'"}]', true); //json_decode($value['email'], true);
                $sendSmtpEmail['cc'] = json_decode('[{"email":"looi@acumenbizcorp.com.sg"}]', true);
                $sendSmtpEmail['htmlContent'] = "<p>Your password was changed.</p><br/><p>Your new password is ".$newlyResetedPassword."</p>";
                ...
                //array_push($pdfDocPath, json_decode($value['attachment'], true));
                
                $sendSmtpEmail['attachment'] = null;
                ...
                try {
                    ...
                    if ($result) 
                    {
                        echo json_encode(array('message' => 'Password reset', 'title' => 'done'));
                    }
                } catch (Exception $e) {
                    echo 'Exception when calling SMTPApi->sendTransacEmail: ', $e->getMessage(), PHP_EOL;
                }
            } else {
                echo 'Password reset is not success';
            }
        }
    }
    ...
    function edit_user($id = NULL, $part = NULL)
    {
        ...
        if ($this->form_validation->run() === TRUE && $this->ion_auth->update($user->id, $data)) {
            ...
            if($_POST['selected_firm'] && $part != "access_client")
            {
            ...
            if($id != $this->session->userdata('user_id') && $part != "access_client")
            {
                $this->save_audit_trail($this->session->userdata("user_id"), "User", "Edit User", strtoupper($this->input->post('first_name'))." ".strtoupper($this->input->post('last_name'))." user is edited.");
                $redirect = "user_page";
            }
            elseif($part == "access_client")
            {
            ...
            {
                $this->save_audit_trail($this->session->userdata("user_id"), "User Profile", "Profile", strtoupper($this->input->post('first_name'))." ".strtoupper($this->input->post('last_name'))." user is edited.");
                $redirect = "homepage";
            }
    ...
    public function save_audit_trail($user_id, $modules, $events, $actions)
    {
        $secretary_audit_trail["user_id"] = $user_id;
