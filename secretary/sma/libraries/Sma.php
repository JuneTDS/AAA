...
class Sma
{
    ...
    public function addzero($value,$count)
    {
		$string = $value;
    ...
    public function remove_time($sdate)
    {
    ...
    public function fed($inv_date)
    {
        if ($inv_date) {
                $date = substr($inv_date, -2) . "/" . substr($inv_date, 5, 2) . "/" . substr($inv_date, 0, 4);
            return $date;
        } else {
            return '0000-00-00';
        }
    ...
    public function send_by_sendinblue($subject, $from_email, $to_email, $cc = null, $message, $attachment = null)
    {
...
$config = SendinBlue\Client\Configuration::getDefaultConfiguration()->setApiKey('api-key', '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');
          ...
          $sender_email = json_decode($from_email, true);
          $sendSmtpEmail['sender'] = $sender_email;
          $sendSmtpEmail['to'] = json_decode($to_email, true);
          if($cc != null)
          {
            $sendSmtpEmail['cc'] = json_decode($cc, true);
          }
          ...
          if($attachment != null)
          {
            $sendSmtpEmail['attachment'] = json_decode($attachment, true);
          }
    ...
    public function send_email($to, $subject, $message, $from = null, $from_name = null, $attachment = null, $cc = null, $bcc = null)
    {
        ...
        $config['useragent'] = "Secretary System";
        ...
        if ($this->Settings->protocol == 'sendmail') {
            ...
            $config['smtp_pass'] = $this->Settings->smtp_pass;//$this->encrypt->decode()
        ...
        //echo json_encode($this->email->initialize($config));
        ...
        if ($attachment) {
            if (is_array(json_decode($attachment))) {
        ...
        if ($this->email->send()) {
            ...
            $email_history["attachment"] = json_encode($attachment);
            $email_history["email_from"] = $from;
            $email_history["email_to"] =  json_encode($to);
            $email_history["status"] = "Success";

            $this->db->insert("email_history",$email_history);
    ...
    public function in_group($check_group, $id = false)
    {
        ...
        //echo json_encode($group->name === $check_group);
        if ($group->name === $check_group) {
            //echo json_encode($check_group);
            return true;
        }
        return false;
    }

    public function user_type($check_user_type, $id = false)
    {
        if ( ! $this->logged_in()) {
            return false;
        }
        $id || $id = $this->session->userdata('user_id');
        $user_type = $this->site->getUserType($id);
        if ($user_type->type_of_user === $check_user_type) {
            return true;
        }
        return false;
    }
	...
	public function remove_comma($d)
	{
		$e= str_replace(',','',$d);
		$f= str_replace(',','',$e);
	...
	public function generate_document($d,$unique_code)
	{
		$id_service_trigger = [];
		$data = [];
		$q = $this->db->query("select A.* from service A, client_service B where A.id = B.service_name and service_start_recurring<=now() and service_end_recurring >=now() and B.service_frequency=0");
		if ($q->num_rows() > 0) {
		...
		foreach($d as $e=>$f)
		{
			foreach($service as $s)
			{
				// ('/are/',$a)
				// echo $f;
				// echo preg_match('/'.$e.'/',$s->field_service);
				if (strpos($s->field_service,$e))
				{
					// print_r($s);
					// print_r($s->field_service);
					$id_service_trigger[$s->id] = $s->service_name;
				}
			}
			// echo "<br/>";
			// echo $e;
		...
		$data['service'] = $id_service_trigger;
		$data['konten'] = $konten;
		// echo json_encode($data);
		foreach($id_service_trigger as $e=>$f)
		{
			$q = $this->db->query("insert into waitingdocument(id_service,document_name,unique_code,content) values(".$e.",'".$f."','".$unique_code."','".json_encode($data)."')");
		}
		// echo "</pre>";
