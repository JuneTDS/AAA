...
class MY_Controller extends CI_Controller {
    ...
    function __construct()
    {
        ...
        if(strtolower($this->router->fetch_method()) != "get_our_service_data")
        {
            ...
            if($this->loggedIn) {
                ...
                //ABn7HP2hPrTyXlM4nyqn3RNXY1PIWODdV1XggsTkVZYcXCCgfb
                //891atJrcLKt10in9neuGp4lZ6d1Jfd2NQjVOPyQs
                //https://developer.intuit.com/v2/OAuth2Playground/RedirectUrl
                //https://acumenbizcorp.com.sg/test_secretary/quickbook_auth/auth_redirect_url_accounting
                //https://bizfiles.com.sg/secretary/quickbook_auth/auth_redirect_url_accounting
                ...
                $this->quickbook_refreshToken = "AB116807529088JrMKa9C8sXitfluYUqlJbqQ8wNE4sVJcWUvU";
                $this->quickbookBaseUrl = "Production";//Production //Development
                $this->quickbookURL = "https://quickbooks.api.intuit.com"; //https://sandbox-quickbooks.api.intuit.com // "https://accounts.intuit.com/"
    ...
    function page_construct($page, $meta = array(), $data = array()) {
        ...
        // echo "file_info  "; print_r($file_info); exit;
        if(count($file_info) > 0)
        {
            ...
            if($file_info[0]["qb_company_id"])
            {
                ...
                // echo "user_qb_access_arr  "; print_r($user_qb_access_arr); exit;
        ...
        // echo "select * 
        // from  qb_access_token_record
        // where qb_company_id = '".$this->session->userdata("qb_company_id")."' AND deleted = 0 AND DATE_ADD(created_at,INTERVAL 1 HOUR) >= NOW() ORDER BY id DESC LIMIT 1"; exit;
        $qb_access_token_record = $qb_access_token_record->result_array();
        // print_r($qb_access_token_record); // exit;
        ...
        else
        {
            ...
            // print_r($qb_access_token_record1); exit;
            if(count($qb_access_token_record1) > 0)
            {
                // print_r("qb_access_token_record1 > 0"); exit;
                $this->session->set_userdata(array(
                    'refresh_token_value'   => $qb_access_token_record1[0]["refresh_token"]
                ));
                // print_r($this->quickbook_clientID); echo "<br/>";
                // print_r($this->quickbook_clientSecret); exit;
        ...
        // echo ("select * 
        //         from user_firm
        //         where firm_id = '".$this->session->userdata("firm_id")."' AND user_id = '".$this->session->userdata("user_id")."' AND in_use  = 1");
        // exit;

        $access_right = $this->db->get();
        $access_right = $access_right->result_array();

        // print_r($access_right); exit;
        ...
        $meta['artemis_password'] = $this->cryptoJsAesEncrypt("Chanel", 'AAALooi1001#'); // 'Looi1001#');
        $meta['artemis_user_pool_id'] = $this->cryptoJsAesEncrypt("Chanel", "ap-southeast-1_DQPYmWYnV");
        $meta['artemis_client_id'] = $this->cryptoJsAesEncrypt("Chanel", "29dsj1f8ejvh0o6jplq38msstm");
        $meta['artemis_region'] = $this->cryptoJsAesEncrypt("Chanel", "ap-southeast-1");
