...
$(document).on('change','.billing_date',function(e){
    showRow();
});
                        ...
                        for(var t = 0; t < service_category_array.length; t++)
                        {
                            ...
                            for(var h = 0; h < service_array.length; h++)
                            {
                                if(category_description == service_array[h]['category_description'])
                                {
                                    ...
                                    option
                                    .attr('data-gst_category_id', service_array[h]['gst_category_id'])
                                    .attr('data-calculate_by_quantity_rate', service_array[h]['calculate_by_quantity_rate'])
                                    .attr('data-gst_new_way', service_array[h]['gst_new_way'])
                                    .attr('data-rate', service_array[h]['rate'])
                                    .attr('data-our_service_id', service_array[h]['service'])
                                    .attr('data-description', service_array[h]['invoice_description'])
                                    .attr('data-currency', service_array[h]['currency'])
                                    .attr('data-unit_pricing', service_array[h]['unit_pricing'])
                                    .attr('data-amount', service_array[h]['amount'])
                                    .attr('value', service_array[h]['id'])
                                    .text(service_array[h]['service_name'])
                                    .appendTo(optgroup);
                                }
                            }
                        }
