...
function resendEmail(id) {
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(response)
            {
                toastr.success("Email is save in que list.", "Email");
                location.reload();
            }
            else
            {
                toastr.error("Email is not save in que list", "Email");
            }
        }				
    });
}
...
function check_connection_button(){
    console.log("check_qb_token", check_qb_token)
    setTimeout(() => {
        if(check_qb_token)
        {
            console.log("if")
...
console.log("qb_company_id", qb_company_id);
if(qb_company_id == "")
{
    ...
    if(valid_setup)
    {
    	...
    	$.ajax({
	        ...
	        success: function(response){
                ...
                if (response == "Duplicate Credit Note") {
                    alert("Credit note number is already exist")
                } else {
                    $('#loadingmessage').hide();
                    $("#currency").attr("disabled", true);
                    if (response.Status === 1) 
            ...
            error: function(error) {
                ...
                if (error.responseText == "Duplicate Credit Note") {
                    $('#loadingmessage').hide();
                    toastr.error("Credit note number is already exist", "Duplicate Credit Note");
                }
                console.log(error)
            }
		});
