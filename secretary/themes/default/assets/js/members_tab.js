...
function refresh()
{
	...
	if ($.fn.DataTable.isDataTable('#datatable_certificate')) {
        $('#datatable_certificate').DataTable().clear().destroy();
    }
...
function share_capital(client_share_capital)
{
	...
	for(var i = 0; i < client_share_capital.length; i++)
	{
		...
		$a += '<div class="td action"><button type="button" class="btn btn-primary edit-btn" onclick="edit_share_capital(this);">Edit</button><button type="button" class="btn btn-primary" style="margin-left: 5px;" onclick="delete_share_capital(this);">Delete</button></div>';
		$a += '</form>';
...
function delete_share_capital(element)
{
...
function edit_share_capital(element)
{
	...
	if(!tr.hasClass("editing")) 
	{
		...
		tr.find("DIV.td").each(function()
		{
			...
			{
				jQuery(this).find("BUTTON.edit-btn").text("Save");
			}
		});
	} 
	else 
...
function certificate_datatable() {
	var t = $('#datatable_certificate').DataTable({
	            "columnDefs": [ {
	                "searchable": false,
	                "orderable": false,
	                'type': 'num', 
