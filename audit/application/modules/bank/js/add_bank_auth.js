...
function Client() {
    ...
    this.getClientName = function() {
        var url = base_url+"/"+folder+"/"+'bank/getClientName';
        //console.log(url);
        var method = "get";
        var data = {};
                ...
                $('#bank_name').select2({
                    ...
                    language: {
                        noResults: function () {
                             ...
                             return "<a href='"+bank_list_url+"'>Bank not exist. Add bank here</a>";
                        }
                    }
                });
                //$(".nationality").prop("disabled",false);
    ...
    this.getAuthStatus = function() {
        ...
        call.send(data, url, method, function(data) {
            //console.log(data);
            $('.auth_status').find("option:eq(0)").remove();
            //console.log(data);
            if(data.tp == 1){
                $.each(data['result'], function(key, val) {
                    var option = $('<option />');
...
$(document).ready(function() {
    ...
    // console.log("nani");
     ...
     $('#create_bank_auth_form').bootstrapValidator({
        ...
        fields: {
            ...
            auth_date: {
                ...
                validators: {
                    notEmpty: {
                        message: 'Authorization date is required and cannot be empty'
                    }
                }
            }
        }
...
$(document).on("submit",function(e){
        ...
        if($('#create_bank_auth_service').css('display') != 'none')
        {
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    if(response.status == "success")
                    {
                        $('.client_name').attr('disabled', true);
                        $('.bank_name').attr('disabled', true);
                        generate_auth(response.bank_auth_id);
...
$(document).on('click',"#saveAuth",function(e){
    var bootstrapValidator = $("#create_bank_auth_form").data('bootstrapValidator');
