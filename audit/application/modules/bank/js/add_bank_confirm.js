...
function Client() {
    ...
    this.getAuthClient = function() {
        var url = base_url+"/"+folder+"/"+'bank/getAuthClient';
        //console.log(url);
        var method = "get";
        var data = {};
        ...
        call.send(data, url, method, function(data) {
            ...
            if(data.tp == 1){
                $.each(data['result'], function(key, val) {
                    ...
                    if(data.selected_auth_client != null && key == data.selected_auth_client)
                    {
    ...
    this.getBankName = function(company_code=null) {
        var url = base_url+"/"+folder+"/"+'bank/getBankNameForUser';
        //console.log(url);
        var method = "get";
    ...
    this.getAuthStatus = function() {
        ...
        var method = "get";
        var data = {};
        $('.confirm_status').find("option:eq(0)").html("Please wait..");
        call.send(data, url, method, function(data) {
            //console.log(data);
            $('.confirm_status').find("option:eq(0)").remove();
            //console.log(data);
            if(data.tp == 1){
                $.each(data['result'], function(key, val) {
                    var option = $('<option />');
...
function runValidate(){
    ...
    $('#create_bank_confirm_form').bootstrapValidator({
...
$(document).on("submit",function(e){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $('.client_name').attr('disabled', true);
                $('.bank_name').attr('disabled', true);
                generate_confirmation(response.bank_confirm_id);
                // console.log("oi");
                window.location.href = bank_url_redirect;
            }
        });
...
function generate_confirmation(confirm_id)
{
    ...
    $.ajax({ //Upload common input
      url       : generate_confirm_document_url,
      type      : "POST",
      dataType  : 'json',
...
$(document).on('click',"#saveConfirm",function(e){
    var bootstrapValidator = $("#create_bank_confirm_form").data('bootstrapValidator');
    bootstrapValidator.validate();
    if(bootstrapValidator.isValid())
        $("#create_bank_confirm_form").submit();
    else return;
    // $("#create_bank_confirm_form").submit();
});
