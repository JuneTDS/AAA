	...
	if(filing)
	{
		...
		for(var i = 0; i < filing.length; i++)
	    {
	       /* total_existing +=  parseInt(transfer_object["current_share"][i]);
	        ...
	        $b += '<td class="hidden"><input type="text" class="form-control" name="each_filing_id" value="'+filing[i]["id"]+'"/></td>';
	        $b += '<td>'+(i+1)+'</td>';
...
$(document).on("click", ".edit_filing", function(){
	
    edit_filing_id =  $(this).data("id");

    console.log("clicked mou");
    ...
    for(var i = 0; i < filing_data.length; i++)
    {
    	if(filing_data[i]["id"] == edit_filing_id)
    	{
			...
			get_financial_year_period(filing_data[i]["financial_year_period_id"]); //get_financial_year_period with id
...
function get_financial_year_period(financial_year_period_id = null)
{
	$.ajax({
		type: "GET",
		url: get_financial_year_period_url,
		dataType: "json",
            ...
            if(data.tp == 1){
                ...
                $.each(data['result'], function(key, val) {
                    ...
                    if(financial_year_period_id != null && key == financial_year_period_id)
                    {
                        ...
                        console.log("not null id ohhohoho");
                    }
...
$('#gst_filing_cycle').change(function(e){
	if($('#gst_year_end').val() != "" && $('#gst_filing_cycle').val() != "0")
    {
    	$.ajax({
			type: "POST",
			url: "client/calculate_new_gst_date",
...
$("#year_end_date").blur(function() {
    ...
    if($('#year_end_date').val() != "")
    {
		...
		else
		{
			...
			$.ajax({
				type: "POST",
				url: calculate_new_filing_date_url,
...
function check_due_date_175($date = null)
{
	if(new Date("8/31/2018") > new Date($('#year_end_date').val()))
    {
		$.ajax({
		    type: "POST",
		    url: "client/check_first_due_date_175",
	...
	$.ajax({ //Upload common input
        ...
        success: function (response) {
            ...
            if (response.Status === 1) {
            	...
            	console.log(response.filing_data);
