...
function Client() {
    ...
    this.getClientName = function() {
        var url = base_url+"/"+folder+"/"+'engagement/getEngagementClient';
        //console.log(url);
        var method = "get";
        var data = {};
...
if(selected_sub_el != "")
{
    ...
    $("#subsequent_el_id").val(selected_sub_el.id);
    // $("#el_uen").val(selected_el.uen);
    $(".new_letter_date").val(selected_sub_el.new_date);
    $(".previous_letter_date").val(selected_sub_el.previous_date);
    $(".start_fye_date").val(selected_sub_el.start_fye_date);
    $(".fye_date").val(selected_sub_el.fye_date);
    $("#director_signing").val(selected_sub_el.director_signing);
    // $(".client_name").trigger("change");
}

function getSubsequentElInterface()
{

    $(".client_name").attr("disabled", false);
    var company_code_selected = $(".client_name :selected").val();
    var selected_el_id = selected_el.id;
    $("#body_subsequent_el tr").remove();
    console.log(company_code_selected);
    $.post(get_subsequent_el_detail_url, {company_code: company_code_selected, el_id: selected_el_id}, function(data){
        $('#loadingMessage').hide();
        if (selected_el != "")
        {
            $(".client_name").attr("disabled", true);
        ...
        var previous_el_info = data['get_previous_el_info'];

        // console.log(data["client_info"][0]);

        if (selected_el == "")
        {
            if(data["client_info"] != false && selected_sub_el == false)
            {
                // $('#el_uen').val(data["client_info"][0]["registration_no"]);

                $('.fye_date').datepicker({ 
                    format: 'dd MM yyyy',
                }).datepicker('setStartDate', "01/01/1920").datepicker('setDate', data["client_info"][0]["fye_date"]);

                $('.previous_letter_date').datepicker({ 
                    format: 'dd MM yyyy',
                }).datepicker('setStartDate', "01/01/1920").datepicker('setDate', data["client_info"][0]["previous_letter_date"]);
        ...
        if(engagement_letter_type_list)
        {
            ...
            for($i = 0; $i < engagement_letter_type_list.length; $i++)
            {
                ...
                // if(data["get_service_proposal_service_info"] && !previous_el_info)
                // {
                ...
                if(previous_el_info)
                {
                    for($b = 0; $b < previous_el_info.length; $b++)
                    {
                        if(previous_el_info[$b]["engagement_letter_type"] == engagement_letter_type_list[$i]["id"])
                        {
                            $("#selected_el_id"+$i).prop('checked', true);
                            $("#hidden_selected_el_id"+$i).val(previous_el_info[$b]["engagement_letter_type"]);
                            $("#currency"+$i).val(previous_el_info[$b]["currency_id"]);
                            $("#fee"+$i).val(addCommas(previous_el_info[$b]["fee"]));
                            $("#unit_pricing"+$i).val(previous_el_info[$b]["unit_pricing"]);
                            $("#servicing_firm"+$i).val(previous_el_info[$b]["servicing_firm"]);
                        }
                    }
                }
            }
...
$(document).on('click',"#submitSubsequentElInfo",function(e){
    $('#loadingmessage').show();
     $(".client_name").attr("disabled", false);

    $.ajax({ //Upload common input
      url: save_subsequent_el_url,
      type: "POST",
      data: $('form#subsequent_el_form').serialize() + '&company_name=' + encodeURIComponent($(".client_name :selected").text()),
      dataType: 'json',
      success: function (response,data) {
        $('#loadingmessage').hide();
        $(".client_name").attr("disabled", true);
          ...
          if (response.status == 1) 
          {
            generate_subsequent_el(response.sub_el_id);
            toastr.success(response.message, response.title);
