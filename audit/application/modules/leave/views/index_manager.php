...
<section class="panel" style="margin-top: 30px;">
	<header class="panel-heading">
		<div class="panel-actions">
			...
			<a class="create_leave themeColor_blue" href="leave/apply_leave/" data-toggle="tooltip" data-trigger="hover" style="height:45px;font-weight:bold;" data-original-title="Create Interview" ><i class="fa fa-plus-circle themeColor_blue" style="font-size:16px;height:45px;"></i> Apply Leave</a>

		</div>
		<h2></h2>
	</header>
	<!-- <div class="pull-right" style="margin: 0.5%;">
		<button class="btn btn_blue" onclick="change_status(true)">Approve Leave</button>
			...
			<div class="tabs">				
				...
				<div class="tab-content clearfix">
									...
									foreach($employee_leave_list as $row)
						  			{
						  				...
						  				echo '<td><button class="btn btn_blue" onclick="withdraw_leave('. $row->id .','. $row->employee_id .','. $row->total_days .','. $row->type_of_leave_id .','. $row->status_id .')" style="margin-bottom:10px;">Withdraw</button></td>';
						  				echo '</tr>';
						  			}
								?>
							</tbody>
					...
					<div id="w2-pending" class="tab-pane">
									...
									foreach($leave_list as $row)
						  			{
						  				...
						  				echo '<td>
						  						<a href="leave/apply_leave/'.$row->id.'">'.$row->leave_no.'</a></td>';
						  				...
						  				{
						  					echo '<td><button class="btn btn_blue" onclick="change_status(true,'. $row->id .','. $row->employee_id .','. $row->type_of_leave_id .')" style="margin-right:10px; margin-bottom:10px;">Approve</button><button class="btn btn_blue" onclick="change_status(false,'. $row->id .','. $row->employee_id .','. $row->type_of_leave_id .')" style="margin-bottom:10px;">Reject</button></td>';
						  				}
						...
						<div style="font-size: 2.4rem;padding: 0; margin: 7px 0 14px 0;">Calender</div>
					...
					<div id="w2-history" class="tab-pane">
						...
						<table class="table table-bordered table-striped mb-none datatable-leave_history" id="datatable-default" style="width:100%;">
							<thead>
								<tr style="background-color:white;">
...
<script>
 ...
 //        });
	...
	function change_status(isApprove, leave_id, employee_id, type_of_leave_id){
	   ...
	   //      	if(confirm("Confirm to APPROVE the selected leave?")){
	   //      		// console.log('hello');
	   //      		$.each($("input[name='select_leave']:checked"), function(){  
	   ...
	   //      			var type_of_leave_id = $(this).parent().parent().find('.type_of_leave_id').val();       
			 ...
			 //        });
	        	...
	        	bootbox.confirm({
			        message: "Confirm to REJECT the selected leave?",
			        closeButton: false,
			        buttons: {
			            confirm: {
			                label: 'Yes',
			        ...
			        callback: function (result) {
			        	if(result == true)
			        	{
			        		//$.each($("input[name='select_leave']:checked"), function(){  
					            ...
					            $.post("<?php echo site_url('leave/change_status'); ?>", { leave_id: leave_id, is_approve: 0, employee_id: employee_id, type_of_leave_id: type_of_leave_id }, function (data, status){
						    			if(status){
						    				window.location = base_url + "leave/index";
						    			}
						    		} 
