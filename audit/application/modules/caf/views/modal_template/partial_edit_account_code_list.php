...
#tbl_edit_sub_account_list tbody tr.selected { 
  background-color: #154069; color: #f5f5f5; 
}
...
<div id="partial_edit_sub_account_list">
  <div id="psal_edit_sub_account_list" class="form-group" style="height: 300px; overflow-y: auto;">
    ...
    <div class="form-group">
      <div class="col-xs-12">
        <input id="search_edit_acc" class="form-control search_input_width" aria-controls="datatable-default" placeholder="Search" type="search" style="float:right;" onkeyup="filter_edit_account()">
      </div>
    </div>
    <div class="form-group">
      <div class="col-xs-12">
          ...
          <tbody>
            ...
            { 
              if($selected_acc_code == $value['account_code'])
              {
                if($value['is_used'] == 1)
                {
                  echo '<tr class="selected used_account" ondblclick="edit_account_code(\'' . $value["account_code"] . '\',\''. $value['fs_default_acc_category_id'].'\')" >';
                }
                else
                {
              ...
              {
                echo '<tr class="used_account init" ondblclick="edit_account_code(\'' . $value["account_code"] . '\',\''. $value['fs_default_acc_category_id'].'\')" >';
              } 
              else { 
            ?>
...
<script type="text/javascript">
  ...
  function filter_edit_account()
  {

    var input, filter, table, tr, td, i, txtValue, txtValue1;
    input = document.getElementById("search_edit_acc");
