...
class Fs_model extends CI_Model
{
    ...
    public function get_fs_company_info($id)
    {
        ...
        if(isset($q[0]['director_signature_1']) && ($q[0]['director_signature_1'] != 0))
        {
        ...
        if(isset($q[0]['director_signature_2']) && $q[0]['director_signature_2'] != 0)
        {
    ...
    public function get_shares($officer_id, $date_of_fye)
    {
        ...
        $q = $this->db->query('SELECT officer.name,
                                ...
                                SUM(member_shares.amount_share) AS end_FY
                                FROM member_shares
    ...
    public function show_investment_in_associate_and_join_ventures($fs_company_info_id)
    {
        ...
        $q = $this->db->query("SELECT * FROM fs_categorized_account WHERE fs_company_info_id =" . $fs_company_info_id);
    ...
    public function calculate_difference_dates($fs_company_info_id)
    {
        $fs_company_info = $this->get_fs_company_info($fs_company_info_id);
        ...
        $start_date_day = date('d', strtotime($current_fye_begin));
        ...
        if($fs_company_info[0]['first_set'])
        {
            ...
            $data["last_fye_end"]    = date('d.m.Y', strtotime($fs_company_info[0]['current_fye_begin']));
            $data["current_fye_end"] = date('d.m.Y', strtotime($current_fye_end));
        }
        else
        {
            if($interval_value_year == 1 && $interval_value_month == 0 && $interval_value_day == 1)
            {
    ...
    public function save_fs_settings($data)
    {
        ...
        if(count($q) > 0)
        {
            $result = $this->db->update("fs_settings", $data, array("id" => $q[0]['id']));
        }
        else
        {
            $result =  $this->db->insert("fs_settings",$data);
        }
    ...
    public function get_fca_id($fs_company_info_id, $account_code)   // account code as known as reference id
    {
        ...
        foreach ($account_code as $ac_key => $ac_value) 
        {
            $q = $this->db->query("SELECT aca.*, fs_default_acc_category.account_code 
                                    ...
                                    WHERE fs_default_acc_category.account_code = '" . $ac_value . "' AND aca.fs_company_info_id=" . $fs_company_info_id);
            $q = $q->result_array();
            ...
            if(isset($value['parent_array']))
            {
                if(count($value['parent_array']) > 0)
                {
                    if(in_array($value['parent_array'][0]['account_code'], $search_account_code_list))
                    {
                        ...
                        {
                            if(isset($data[$key]['parent_array'][0]['total_c']) && isset($operate_values_list[$sac_key]['insert_values_arr']['total_c']))
                            {
                                $data[$key]['parent_array'][0]['total_c']     += $operate_values_list[$sac_key]['insert_values_arr']['total_c'];
                                $data[$key]['parent_array'][0]['total_c_adjusted']     += $operate_values_list[$sac_key]['insert_values_arr']['total_c_adjusted'];
    ...
    public function find_account_in_array($data, $search_account_code_list)   // operate (+ - x /) to specific account with account code
    {
        ...
        foreach ($data as $key => $value) 
        {
            ...
            if(isset($value['parent_array']))
            {
                if(count($value['parent_array']) > 0)
                {
                    ...
                    {
                        $sac_key = array_search($value['parent_array'][0]['account_code'], $search_account_code_list);
                        // array_push($temp_data, $data[$key]['parent_array'][0]);
                        // print_r($data[$key]['parent_array'][0]);
                        // return "HELLO?";

                        return $data[$key]['parent_array'][0];
            ...
            if(isset($value['child_array']))
            {
                if(count($value['child_array']) > 0)
                {
                    if(!isset($value['child_array']['value']))
                    {
                        // print_r($search_account_code_list);
                        $temp_data = $this->find_account_in_array($value['child_array'], $search_account_code_list);
                        // print_r($temp_data);
    ...
    public function change_sign_in_account($data)   // Flip the sign of account (Credit has no bracket, Debit has bracket / POsitive to Negative - , Negative to Positive + )
    {
        foreach ($data as $key => $value) 
        {
            if(isset($value['parent_array']))
            {
                if(count($value['parent_array']) > 0 )
                {
                    $data[$key]['parent_array'][0]['value']                     = (double)$value['parent_array'][0]['value']                     * -1;
                    $data[$key]['parent_array'][0]['company_end_prev_ye_value'] = (double)$value['parent_array'][0]['company_end_prev_ye_value'] * -1;
                    $data[$key]['parent_array'][0]['company_beg_prev_ye_value'] = (double)$value['parent_array'][0]['company_beg_prev_ye_value'] * -1;
                    if(isset($value['parent_array'][0]['total_c'] ))
                    {
                        $data[$key]['parent_array'][0]['total_c']       = (double)$value['parent_array'][0]['total_c']      * -1;
                    }
                    if(isset($value['parent_array'][0]['total_c_adjusted'] ))
                    {
                        $data[$key]['parent_array'][0]['total_c_adjusted']       = (double)$value['parent_array'][0]['total_c_adjusted']      * -1;
                    }
                    if(isset($value['parent_array'][0]['total_c_lye']))
                    {
                        $data[$key]['parent_array'][0]['total_c_lye']   = (double)$value['parent_array'][0]['total_c_lye']  * -1;
                    }
            ...
            if(isset($value['child_array']))
            {
                if(count($value['child_array']) > 0)
                {
                    if(isset($value['child_array']['value']))
                    {
                        $data[$key]['child_array']['value']                     = (double)$value['child_array']['value']                     * -1;
                        $data[$key]['child_array']['adjusted_value']            = (double)$value['child_array']['adjusted_value']            * -1;
                        $data[$key]['child_array']['company_end_prev_ye_value'] = (double)$value['child_array']['company_end_prev_ye_value'] * -1;
                        $data[$key]['child_array']['company_beg_prev_ye_value'] = (double)$value['child_array']['company_beg_prev_ye_value'] * -1;
    ...
    public function is_saved_fs_categorized_account($assignment_id)
    {
        $q = $this->db->query("SELECT * FROM audit_categorized_account WHERE assignment_id = " . $assignment_id);
        $q = $q->result_array();
