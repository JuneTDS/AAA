...
class Fs_notes_model extends CI_Model
{
    ...
    //     {
    //         $q = $this->db->query("SELECT aca.*, fs_default_acc_category.account_code FROM audit_categorized_account aca 
    //                                 LEFT JOIN fs_default_acc_category ON aca.fs_default_acc_category_id = fs_default_acc_category.id 
    //                                 WHERE fs_default_acc_category.account_code = '" . $ac_value . "' AND fs_company_info_id=" . $fs_company_info_id);
    //         $q = $q->result_array();
    ...
    public function get_fs_sub_intangible_assets($fs_company_info_id)
    {
        ...
        $q = $this->db->query("SELECT * FROM fs_sub_intangible_assets ia WHERE fs_company_info_id=" . $fs_company_info_id . " ORDER BY ia.order_by");
        $q = $q->result_array();
                    ...
                    foreach ($fs_ntfs['ntfs'][$matched_fs_ntfs_key]['compare_ref_id'] as $cri_key => $compare_ref_id) // full formula "A001 < L100"
                    {
                        ...
                        foreach ($split_formula as $key1 => $formula_data) 
                        {
                            ...
                            else
                            {
                                ...
                                // print_r(array($fs_company_info_id, $formula_data));
