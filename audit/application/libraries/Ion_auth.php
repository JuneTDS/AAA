...
class Ion_auth
{
            ...
            if ($user) {
                ...
                // } else {
                    ...
                    $parse_data = array(
                        ...
                        'site_name' => 'PAYROLL SYSTEM',
                        'logo' => '<img src="' . base_url() . 'assets/logo/logo.png" alt="PAYROLL SYSTEM"/>'
                    );
                    ...
                    $subject = 'Forgot Password - PAYROLL SYSTEM';
    ...
    public function register($username, $user_type_id, $password, $email, $additional_data = array(), $term_and_condition, $active = FALSE, $notify = FALSE, $selected_firm = FALSE)
    { //need to test email activation
        ...
        if (!$email_activation || $active == '1') {
            $id = $this->auth_model->register($username, $user_type_id, $password, $email, $additional_data, $term_and_condition, NULL, $active, $selected_firm);
            if ($id !== FALSE) {
                if ($notify) {
                    $this->load->library('parser');
                    $parse_data = array(
                    ...
                    // echo json_encode(file_get_contents('./application/modules/auth/email_templates/credentials.html'));
                    $msg = file_get_contents('./application/modules/auth/email_templates/credentials.html');
                    
                    $message = $this->parser->parse_string($msg, $parse_data);
                    $subject = 'New User Created - ' . 'PAYROLL SYSTEM';
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $message = $this->parser->parse_string($msg, $parse_data);
                $subject = "Email Activation" . ' - ' . $this->Settings->site_name;
                //$this->sma->send_email($email, $subject, $message);
