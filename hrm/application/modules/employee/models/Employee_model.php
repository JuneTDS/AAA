...
class Employee_model extends CI_Model
{
    ...
    public function get_employeeList($user_id = NULL,$is_manager = NULL){
        ...
        {
            $employee_id = $this->get_employee_id_from_user_id($user_id);

            $employee_data = $this->get_staff_info($employee_id)[0];

            // $q = $this->db->query("SELECT e.*, ue.user_id AS `user_id`, u.email AS `user_email`,payroll_offices.office_name, department.department_name,payroll_employee_telephone.telephone FROM payroll_employee e LEFT JOIN payroll_user_employee ue ON ue.employee_id = e.id LEFT JOIN users u ON ue.user_id = u.id LEFT JOIN department ON department.id = e.department LEFT JOIN payroll_offices ON payroll_offices.id = e.office LEFT JOIN payroll_employee_telephone ON payroll_employee_telephone.employee_id = e.id AND payroll_employee_telephone.primary_telephone = 1 WHERE ue.user_id ='". $user_id ."' GROUP BY e.id ORDER BY e.name");

            if ($employee_data->id == null) {
            $q = $this->db->query("SELECT e.*, ue.user_id AS `user_id`, u.email AS `user_email`,payroll_offices.office_name, 
                    department.department_name,payroll_employee_telephone.telephone, `payroll_designation`.`sorting`
                FROM payroll_employee e 
                LEFT JOIN payroll_user_employee ue ON ue.employee_id = e.id 
                LEFT JOIN users u ON ue.user_id = u.id 
                LEFT JOIN department ON department.id = e.department 
                LEFT JOIN payroll_offices ON payroll_offices.id = e.office 
                LEFT JOIN payroll_employee_telephone ON payroll_employee_telephone.employee_id = e.id AND payroll_employee_telephone.primary_telephone = 1 
                LEFT JOIN `payroll_designation` ON `payroll_designation`.`designation` = e.`designation`
                ...
                LEFT JOIN payroll_employee_telephone ON payroll_employee_telephone.employee_id = e.id AND payroll_employee_telephone.primary_telephone = 1 
                LEFT JOIN `payroll_designation` ON `payroll_designation`.`designation` = e.`designation`
                WHERE (e.`department` = ".$employee_data->department." AND `payroll_designation`.`sorting` < ".$employee_data->designation_sorting.") OR
                    (e.`department` = ".$employee_data->department." AND `payroll_designation`.`sorting` = ".$employee_data->designation_sorting." AND user_id = ".$user_id.")
                GROUP BY e.id ORDER BY e.name");
            }

            // SELECT e.*, ue.user_id AS `user_id`, u.email AS `user_email`,payroll_offices.office_name, department.department_name,payroll_employee_telephone.telephone, `payroll_designation`.`sorting`
            // FROM payroll_employee e 
            // LEFT JOIN payroll_user_employee ue ON ue.employee_id = e.id 
            // LEFT JOIN users u ON ue.user_id = u.id 
            // LEFT JOIN department ON department.id = e.department 
            ...
            // WHERE (e.`department` = 4 AND `payroll_designation`.`sorting` < 3) OR
            // (e.`department` = 4 AND `payroll_designation`.`sorting` = 3 AND user_id = 176)
            // GROUP BY e.id  
            // ORDER BY `payroll_designation`.`sorting` ASC
        }
    ...
    public function get_staff_info($staff_id){
        ...
        //  $q = $this->db->query(" SELECT payroll_employee.*, GROUP_CONCAT(DISTINCT CONCAT(payroll_employee_telephone.id,',', payroll_employee_telephone.telephone, ',', payroll_employee_telephone.primary_telephone)SEPARATOR ';') AS 'employee_telephone', payroll_offices.office_countryFROM payroll_employee LEFT JOIN payroll_employee_telephone ON payroll_employee_telephone.employee_id = payroll_employee.id LEFT JOIN payroll_offices ON payroll_offices.id = payroll_employee.office WHERE payroll_employee.id = '".$staff_id."' ORDER BY payroll_employee_telephone.primary_telephone DESC ");

        $q = $this->db->query(" SELECT payroll_employee.*, GROUP_CONCAT(DISTINCT CONCAT(payroll_employee_telephone.id,',', payroll_employee_telephone.telephone, ',', payroll_employee_telephone.primary_telephone)SEPARATOR ';') AS 'employee_telephone',
            payroll_offices.office_country, users.group_id, `payroll_designation`.`sorting` as designation_sorting
            ...
            WHERE payroll_employee.id = '".$staff_id."' ORDER BY payroll_employee_telephone.primary_telephone DESC ");
