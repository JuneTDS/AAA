...
class Auth_model extends CI_Model
{
    ...
    public function reset_password($identity, $new)
    {
        ...
        $this->trigger_events('extra_where');
        $this->db->update($this->tables['users'], $data, array($this->identity_column => $identity, "user_deleted" => 0));
    ...
    public function change_password($identity, $old, $new)
    {
        ...
        if ($old_password_matches === TRUE) {
            ...
            $successfully_changed_password_in_db = $this->db->update($this->tables['users'], $data, array($this->identity_column => $identity, "user_deleted" => 0));
    ...
    public function username_check($username = '')
    {
        ...
        return $this->db->where(array('username' => $username, 'user_deleted' => 0))
    ...
    public function email_check($email = '')
    {
        ...
        $this->trigger_events('extra_where');
        //array('email' => $email, 'user_deleted = 0')
        return $this->db->where(array('email' => $email, 'user_deleted' => 0))
            ->count_all_results($this->tables['users']) > 0;
    }
    ...
    public function forgotten_password($identity)
    {
        ...
        $this->db->update($this->tables['users'], $update, array($this->identity_column => $identity, "user_deleted" => 0));
    ...
    public function register($username, $user_type_id, $password, $email, $additional_data = array(), $term_and_condition,  $activation_code, $active = FALSE, $selected_firm = FALSE, $selected_person = FALSE)
    {
        ...
        if ($this->identity_column == 'email' && $this->email_check($email)) {
            $this->set_error('This email register under this system already.');
            return FALSE;
        } 
        ...
        if($additional_data["group_id"] != "4")
        {
            if (!isset($additional_data['group_id'])) {
                ...
                $query_admin_code_num = $this->db->query("select MAX(CAST(SUBSTRING(admin_code,5, length(admin_code)-4) AS UNSIGNED)) as admin_code from user_admin_code");
                //echo json_encode($query_admin_code_num->result_array());
                $query_admin_code = $query_admin_code_num->result_array();

                if ($query_admin_code_num->num_rows() > 0 && $query_admin_code[0]["admin_code"] != null) 
                {
                    
                    //$array_invoice_no = explode('-', $query_invoice_no[0]["invoice_no"]);
                    $last_section_admin_code = (int)$query_admin_code[0]["admin_code"] + 1;
                    $admin_code = date("Y").str_pad($last_section_admin_code,3,"0",STR_PAD_LEFT);

                }
                else
                {   //$admin_code = $query_admin_code->result_array();
                    $admin_code = date("Y").str_pad(1,3,"0",STR_PAD_LEFT);
                    //echo json_encode($query_admin_code->result_array());

                }

                $user_admin_code['admin_code'] = $admin_code;
                $this->db->insert('user_admin_code', $user_admin_code);

                $admin_code_id = $this->db->insert_id();
                $this->session->set_userdata('user_admin_code_id', $admin_code_id);
                // echo json_encode($this->session->userdata('user_admin_code_id'));
                $additional_data['user_admin_code_id'] = $admin_code_id;
                $additional_data['group_id'] = 2;
                $additional_data['total_no_of_user'] = 100;
                $additional_data['date_of_registration'] = date('Y-m-d');
                $additional_data['date_of_expiry'] = date('Y-m-d', strtotime('+3 year'));
                $additional_data['total_no_of_client'] = 1000;
            }
            else
            {
                $this->db->select('*');
                ...
                $additional_data['user_admin_code_id'] = $user_info[0]['user_admin_code_id'];
                $additional_data['date_of_registration'] = $user_info[0]['date_of_registration'];
                $additional_data['date_of_expiry'] = $user_info[0]['date_of_expiry'];
                $additional_data['no_of_client'] = $user_info[0]['no_of_client'];
                $additional_data['total_no_of_client'] = 1000;
                // $additional_data['no_of_firm'] = $user_info[0]['no_of_firm'];
                $additional_data['no_of_firm'] = 2;

                if($additional_data['group_id'] == 2)
                {
                    $additional_data['total_no_of_user'] = $user_info[0]['total_no_of_user'];
        ...
        else
        {
            ...
            $additional_data['date_of_registration'] = $user_info[0]['date_of_registration'];
            $additional_data['date_of_expiry'] = $user_info[0]['date_of_expiry'];
            $additional_data['user_admin_code_id'] = $this->session->userdata('user_admin_code_id');
        }
        ...
        if($user_type_id == 2)
        {
            $firm['name'] = "Acumen Bizcorp Pte Ltd";
            $firm['postal_code'] = "069542";
            $firm['street_name'] = "143 Cecil Street";
            $firm['building_name'] = "GB Building";
            $firm['unit_no1'] = "16";
            $firm['unit_no2'] = "03";
            $firm['gst_checkbox'] = 0;
            $firm['no_gst_date'] = date('d/m/Y', time());
            $firm['file_name'] = "secretary_logo_c60_icon4.ico";
        ...
        // {
        //     $now = getDate();
        //     $user_admin_code['user_id'] = $id;
        //     $user_admin_code['admin_code'] = 'admin_'.$now[0];
        ...
        if($additional_data["group_id"] != "4")
        {
            if($selected_firm)
            {
                //$user_firm["admin_code"] = $this->session->userdata('admin_code');
                $user_firm['user_admin_code_id'] = $this->session->userdata('user_admin_code_id');
                $user_firm["user_id"] = $id;
                $user_firm["client_module"] = "full";
                $user_firm["person_module"] = "full";
        ...
        else
        {
            ...
            for($r = 0; $r < count($selected_firm); $r++)
            {
                $user_client["client_id"] = $selected_firm[$r];

                $this->db->insert('user_client', $user_client);
            }

            $query_client = $this->db->query("select * from client where id = '".$selected_firm[0]."'");
        ...
        if(isset($id) && !(isset($additional_data["interview_flag"])))
        // if(isset($id))
        {
            ...
            $this->set_user_employee($id, $employee_id);
        }

        return (isset($id)) ? $id : FALSE;
    }

    // public function register($username, $user_type_id, $password, $email, $additional_data = array(), $term_and_condition,  $activation_code, $active = FALSE, $selected_firm = FALSE)
    // {
    //     $this->trigger_events('pre_register');
    ...
    //             $additional_data['date_of_registration'] = date('Y-m-d');
    //             $additional_data['date_of_expiry'] = date('Y-m-d', strtotime('+3 year'));
    //             $additional_data['total_no_of_client'] = 500;
    //         }
    //         else
    //         {
    //             $this->db->select('*');
    ...
    public function set_user_employee($user_id, $employee_id){
        $q = $this->db->query("SELECT * FROM payroll_user_employee WHERE employee_id='" . $employee_id . "' AND user_id='" . $user_id . "'");
        $data = array(
        ...
        if(!$q->num_rows()){
            $this->db->insert('payroll_user_employee', $data);    // insert new customer to database
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        // echo $identity;
        $this->trigger_events('extra_where');
        // echo $this->identity_column;
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, last_ip_address, first_name, last_name, avatar, gender, group_id, date_of_expiry', 'user_deleted')
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                if ($user->active != 1) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('Account is not activated');
                    return FALSE;
                }

                if($user->group_id != 1)
                {
                    if(strtotime($today) > strtotime($user->date_of_expiry))
                    {
                        $this->trigger_events('post_login_unsuccessful');
                        $this->set_error('Your account has expired. Click <a href="#">here</a> to renew.');
                        return FALSE;
                    }
                }
    ...
    public function users($groups = NULL, $part = NULL)
    {
        ...
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            ...
            if($part == "access_client")
            {
                $this->db->select(array(
                    ...
                    'GROUP_CONCAT(user_client.client_id) AS client_id'
                ));

                $this->db->join('user_client', 'user_client.user_id = '. $this->tables['users'] . '.id', 'left');
            ...
            else
            {
                $this->db->select(array(
                    ...
                    $this->tables['users'] . '.id as id',
                    $this->tables['users'] . '.id as user_id',
                    'GROUP_CONCAT(user_firm.firm_id) AS firm_id'
                ));

                $this->db->join('user_firm', 'user_firm.user_id = '. $this->tables['users'] . '.id', 'left');
            }
        }
       ...
       // $this->response = $this->db->get();
        ...
        //return json_encode(array("users" => "abc", "user_rules" => "abc"));
    }

    public function user($id = NULL, $part = NULL)
    {
    ...
    public function delete_user($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update($this->tables['users'], array('user_deleted' => 1));
    ...
    public function login_remembered_user()
    {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, username, email, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, show_cost, show_price')
...
END) as percent")
    ...
    public function get_rules_info()
    {
        $q = $this->db->query("select user_rules.* from user_rules where deleted = 0 order by type ASC");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function get_users_group_dropdown(){
        // if($user_group_name == 'admin'){
        //     $q = $this->db->query("SELECT * FROM groups");
        // }else{
            $q = $this->db->query("SELECT * FROM groups WHERE name NOT IN ('admin', 'owner', 'client')");
        // }

        $user_group_list = array();
        $user_group_list[''] = 'Select a user group';

        foreach($q->result_array() as $item){
            $user_group_list[$item['id']] = $item['description']; 
        }

        return $user_group_list;
    }

    public function check_authentication($employee_id, $is_manager){
        if(!$is_manager)
        {
            $q = $this->db->query("SELECT * FROM payroll_user_employee WHERE user_id = ".$this->session->userdata("user_id"));
            $user_employee = $q->result();
            $user_employee = $user_employee[0];
    ...
    public function check_admin_access($staff_id){
        $q = $this->db->query("SELECT * FROM `users` WHERE `user_type_id` = 1 AND `group_id` = 2 AND `department_id` IN (1,2,3,4,6) AND id = ".$this->session->userdata("user_id"));
