...
class Budget_hours_model extends CI_Model
{
    ...
    public function get_employee_assignment_list($id){
        ...
        foreach ($employeeList as $key => $value) {
            // print_r($value->name);
            if ($PICClause != "(") {
                $PICClause .= " or PA.PIC LIKE '%".$value->name."%'";
            } else {
                $PICClause .= "PA.PIC LIKE '%".$value->name."%'";
            }
            // echo "<br/><br/>";
        }
        $PICClause .= ")";
        ...
        $query = $this->db->query(" SELECT PA.* FROM payroll_assignment PA WHERE PA.type_of_job != 13 AND PA.status != 10 AND PA.deleted = 0 AND ".$PICClause);

        // echo " SELECT PA.* FROM payroll_assignment PA WHERE PA.type_of_job != 13 AND PA.status != 10 AND PA.deleted = 0 AND ".$PICClause;
