...
class Timesheet_model extends CI_Model
{
    ...
    public function get_all_timesheet($user_id, $is_manager){
        if ($is_manager) {
            ...
            $employee_id = $this->employee_model->get_employee_id_from_user_id($user_id);

            $employee_data = $this->employee_model->get_staff_info($employee_id)[0];

            $q = $this->db->query("SELECT t.*, e.name AS `employee_name` 
                FROM timesheet t 
                LEFT JOIN payroll_employee e ON e.id = t.employee_id
                LEFT JOIN `payroll_designation` ON `payroll_designation`.`designation` = e.`designation`
                WHERE year(t.month)= year(CURRENT_TIMESTAMP) AND 
                (e.`department` = ".$employee_data->department." AND `payroll_designation`.`sorting` < ".$employee_data->designation_sorting.") OR 
                (e.`department` = ".$employee_data->department." AND `payroll_designation`.`sorting` = ".$employee_data->designation_sorting." AND t.employee_id = ".$employee_id.")
                GROUP BY t.timesheet_no
                ORDER BY month(t.month) DESC");
        }
