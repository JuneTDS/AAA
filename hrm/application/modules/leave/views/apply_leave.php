				    ...
				    <div class="row">
				        <div class="col-lg-12">
				            <!-- <form id="appy_leave" method="POST" autocomplete="off"> -->
								...
								<div class="form-group row">
									<label for="mc_reason" class="col-sm-3 col-form-label">Your Country: </label>
									<div class="col-sm-5">
										<input type="radio" name="holiday_type" value="1" checked /> Singapore
										<input type="radio" name="holiday_type" value="2" style="margin-left: 20px" /> Malaysia
									</div>
								</div>
								<div class="form-group row">
...
/* --------------------------------------------------------------------------------------------------------- */
	...
	$("input[name=holiday_type]").on("change", function (e) {
		calculate_totalDays();	// excluding weekend.
    });
...
$("#leave_child_dob").on("dp.change" , function (e)
{
    ...
    $(".appy_leave").click(function(){
		...
		if(($("#reason").val()).trim() == '')
		{
			toastr.error('Please Enter the Reason for Leave', 'Reminder');
			flag = false;
		}
    ...
    function calculate_totalDays(){
		...
		dates.holiday_type 		= 1; 
		
		let ele = $('input[name=holiday_type]');
		for (i = 0; i < ele.length; i++) {
			if (ele[i].checked) {
				dates.holiday_type = parseInt(ele[i].value);
			}
		}
