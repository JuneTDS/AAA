		...
		<div class="col-md-12">
			<div class="tabs">				
				<ul class="nav nav-tabs nav-justify">
					...
					<li class="check_state" data-information="history" style="display:none;">
							...
							<tbody>
								<?php
									foreach($leave_list as $row)
						  			{
										$remaining_al = 0;
										foreach($leave_balance_list as $leave_balance) {
											if ($row->employee_id == $leave_balance->employee_id &&
												$row->type_of_leave_id == $leave_balance->type_of_leave_id) {
													$remaining_al = $leave_balance->annual_leave_days;
											}
										}
										...
										echo '<td>'.$row->reason.'</td>';
						  				...
						  				echo '<td>'.$remaining_al.'</td>';

						  				if($row->status == '1'){
					  						echo '<td>
					  								<button class="btn btn_purple" onclick="change_status(true,'. $row->id .','. $row->employee_id .','. $row->type_of_leave_id .','. $row->total_days .','. $remaining_al .')" style="margin-right:10px; margin-bottom:10px;">Approve</button>

					  								<button class="btn btn_purple" onclick="change_status(false,'. $row->id .','. $row->employee_id .','. $row->type_of_leave_id .','. $row->total_days .','. $remaining_al .')" style="margin-bottom:10px;">Reject</button>
					  								</td>';
					  					}
					  					else{
