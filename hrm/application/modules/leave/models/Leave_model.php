...
class Leave_model extends CI_Model
{
    ...
    public function get_employee_leaveList($employee_id, $fromdate = null, $todate = null){
        $emp_id = $employee_id;
        if (gettype($employee_id) == "object") {
            $emp_id = $employee_id->id;
        }

        if ($fromdate != null && $todate != null) {
            $q = $this->db->query("SELECT l.*, l.status as status_id, payroll_type_of_leave.leave_name, 
                payroll_employee.name as employee_name, firm.name as firm_name
                FROM `payroll_leave` l 
                LEFT JOIN payroll_type_of_leave ON l.type_of_leave_id = payroll_type_of_leave.id 
                LEFT JOIN payroll_employee ON l.employee_id = payroll_employee.id 
                LEFT JOIN `firm` ON payroll_employee.firm_id = firm.id
                -- LEFT JOIN `payroll_employee_annual_leave` ON payroll_employee.id = payroll_employee_annual_leave.employee_id
                WHERE (l.status=1 OR (l.status=2 AND l.end_date <= CURRENT_DATE))
                AND ((`l`.`start_date` >= '".$fromdate."' AND `l`.`start_date` <= '".$todate."')
                OR (`l`.`end_date` >= '".$fromdate."' AND `l`.`end_date` <= '".$todate."')
                OR (`l`.`start_date` <= '".$fromdate."' AND `l`.`end_date` >= '".$todate."'))
                AND l.employee_id='". $emp_id ."' ORDER BY l.status ASC");
        } else {
            $q = $this->db->query("SELECT l.*, l.status as status_id, payroll_type_of_leave.leave_name, 
                payroll_employee.name as employee_name, firm.name as firm_name
                FROM `payroll_leave` l 
        ...
        //     FROM `payroll_leave` l 
        //     LEFT JOIN payroll_type_of_leave ON l.type_of_leave_id = payroll_type_of_leave.id
        //     LEFT JOIN `payroll_employee` ON `payroll_employee`.`id` = l.`employee_id`
        //     LEFT JOIN `payroll_designation` ON `payroll_designation`.`designation` = `payroll_employee`.`designation`
        //     WHERE `payroll_employee`.`department` = ".$employee_data->department." AND `payroll_designation`.`sorting` <= ".$employee_data->designation_sorting." ORDER BY l.status ASC");
    ...
    public function get_leave_type_list() {
        $q = $this->db->query("SELECT * FROM `payroll_type_of_leave`;");
        return $q->result();
    }

    public function get_employee_dayoffList($employee_id, $fromdate = null, $todate = null){
        $emp_id = $employee_id;
        if (gettype($employee_id) == "object") {
            $emp_id = $employee_id->id;
        }
        if ($fromdate != null && $todate != null) {
            $q = $this->db->query("SELECT al.*, payroll_type_of_leave.leave_name, 
                payroll_employee.name as employee_name, firm.name as firm_name
                FROM `payroll_employee_annual_leave` al  
                LEFT JOIN payroll_employee ON payroll_employee.id = al.employee_id
                LEFT JOIN `firm` ON payroll_employee.firm_id = firm.id
                LEFT JOIN payroll_type_of_leave ON payroll_type_of_leave.id = al.type_of_leave_id
                WHERE (`al`.`last_updated` >= '".$fromdate."' AND `al`.`last_updated` <= '".$todate."')
                AND al.employee_id='". $emp_id ."' ORDER BY al.id DESC");
        } else {
            $q = $this->db->query("SELECT al.*, payroll_type_of_leave.leave_name, 
                payroll_employee.name as employee_name, firm.name as firm_name
                FROM `payroll_employee_annual_leave` al  
                ...
                LEFT JOIN `firm` ON payroll_employee.firm_id = firm.id
                LEFT JOIN payroll_type_of_leave ON payroll_type_of_leave.id = al.type_of_leave_id
                WHERE al.employee_id='". $emp_id ."' ORDER BY al.id DESC");
        }
        return $q->result();
    }

    public function get_dayoffList($filterData = null){
        $condition = "";
        if ($filterData != null) {
            if (isset($filterData["employee"]) && $filterData["employee"] != "") {
                $condition .= " WHERE al.employee_id = ".$filterData["employee"];
            }

            if (isset($filterData["leave_type"]) && $filterData["leave_type"] != "") {
                if ($condition == "") {
                    $condition .= " WHERE al.type_of_leave_id = ".$filterData["leave_type"];
                } else {
                    $condition .= " AND al.type_of_leave_id = ".$filterData["leave_type"];
                }
            }

            if (isset($filterData["fromdate"]) && $filterData["fromdate"] != "" &&
                isset($filterData["todate"]) && $filterData["todate"] != "") {
                    if ($condition == "") {
                        $condition .= " WHERE (`al`.`last_updated` >= '".$filterData["fromdate"]."' AND `al`.`last_updated` <= '".$filterData["todate"]."')";
                    } else {
                        $condition .= " AND (`al`.`last_updated` >= '".$filterData["fromdate"]."' AND `al`.`last_updated` <= '".$filterData["todate"]."')";
                    }
            }
        }
        $q = $this->db->query("SELECT al.*, payroll_type_of_leave.leave_name, 
    ...
    public function get_leaveList($filterData = null){    // for admin side

        $condition = "";
        if ($filterData != null) {
            if (isset($filterData["employee"]) && $filterData["employee"] != "") {
                $condition .= " AND e.id = ".$filterData["employee"];
            }

            if (isset($filterData["leave_type"]) && $filterData["leave_type"] != "") {
                $condition .= " AND l.type_of_leave_id = ".$filterData["leave_type"];
            }

            if (isset($filterData["fromdate"]) && $filterData["fromdate"] != "" &&
                isset($filterData["todate"]) && $filterData["todate"] != "") {
                $condition .= " AND ((`l`.`start_date` >= '".$filterData["fromdate"]."' AND `l`.`start_date` <= '".$filterData["todate"]."')
                    OR (`l`.`end_date` >= '".$filterData["fromdate"]."' AND `l`.`end_date` <= '".$filterData["todate"]."')
                    OR (`l`.`start_date` <= '".$filterData["fromdate"]."' AND `l`.`end_date` >= '".$filterData["todate"]."'))";
            }
        }

        $q = $this->db->query("SELECT l.*, e.id AS `employee_id`, e.name AS `employee_name`, f.id AS `firm_id`, f.name AS `firm_name`, ptl.leave_name, ue.user_id FROM `payroll_leave` l 
        ...
        -- LEFT JOIN payroll_employee_annual_leave eal ON eal.employee_id = e.id AND eal.type_of_leave_id = l.type_of_leave_id 
        -- AND eal.last_updated = (SELECT MAX(eal_2.last_updated) FROM payroll_employee_annual_leave eal_2 WHERE eal_2.employee_id = e.id AND eal_2.type_of_leave_id = l.type_of_leave_id)
        LEFT JOIN `payroll_type_of_leave` ptl ON ptl.id = l.type_of_leave_id
        WHERE (l.status=1 OR (l.status=2 AND l.end_date <= CURRENT_DATE))".$condition."
        ORDER BY l.status");
    ...
    public function get_leave_balance_list() {
        $q = $this->db->query("SELECT * FROM payroll_employee_annual_leave eal WHERE eal.id IN (SELECT MAX(id) as id FROM payroll_employee_annual_leave eal_2 GROUP BY `employee_id`,`type_of_leave_id`);");
        return $q->result();
    }

    public function get_leaves(){    // for admin side
        $q = $this->db->query("SELECT `id`, `leave_name`, `days` from payroll_type_of_leave ORDER BY id");
    ...
    {    
        ...
        $cc                 = json_encode(array(array("email"=> "hr@aaa-global.com")));
        $message            = $this->parser->parse_string($msg, $parse_data,true);
