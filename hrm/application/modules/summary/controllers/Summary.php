...
class Summary extends MX_Controller
{
    ...
    {   
        ...
        {
            $employee_id = $this->employee_model->get_employee_id_from_user_id($this->data['User']);
            $employee_data = $this->employee_model->get_staff_info($employee_id)[0];

            $this->data['updated_or_completed_list'] = $this->summary_model->get_updated_or_completed_list($employee_data, $this->data['User']);
            $this->data['job_remain_list'] = $this->summary_model->get_job_remain_list($employee_data, $this->data['User']);
        } else {
