...
class Sma
{
    ...
    public function send_email($to, $subject, $message, $from = null, $from_name = null, $attachment = null, $cc = null, $bcc = null)
    {
        ...
        if ($from && $from_name) {
            ...
            $this->email->from($from, 'HRM SYSTEM');
        } else {
            $this->email->from($this->Settings->default_email, 'HRM SYSTEM');
        }
...
$config = SendinBlue\Client\Configuration::getDefaultConfiguration()->setApiKey('api-key', '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');
    ...
    {
        $q = $this->db->query('SELECT payroll_employee.designation FROM payroll_employee INNER JOIN payroll_user_employee ON payroll_user_employee.employee_id = payroll_employee.id WHERE payroll_user_employee.user_id = "'.$id.'"');

        if($q->num_rows())
        {
            $q = $q->result();

            return $q[0]->designation;
        }
        else
        {
            return false;
